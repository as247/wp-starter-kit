...
class BladeCompiler extends Compiler implements CompilerInterface
{
     ...
     *
     * @param  \WpStarter\View\Component  $component
     * @return string
     */
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php elseif (\WpStarter\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\WpStarter\Support\Facades\Blade::check('{$name}')): ?>";
        });
